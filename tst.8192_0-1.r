...
#Rprof(append = TRUE)
myrank=comm.rank();
fnamev=paste("8192_0-1/outV",myrank,sep=".");

x = comm.fread ("auth8192", pattern="*",readers=512, quote="",sep=";",header=F)
barrier()
comm.print("read all");
...
fwrite(x[,c("a","un")], file=paste("8192_0-1/outL",myrank,sep="."), sep=";",col.names=FALSE,quote=FALSE,append=F);
barrier()
