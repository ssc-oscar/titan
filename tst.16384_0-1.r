...
#Rprof(append = TRUE)
myrank=comm.rank();
fnamev=paste("16384_0-1/outV",myrank,sep=".");

x = comm.fread ("auth16384", pattern="*",readers=1024, quote="",sep=";",header=F)
barrier()
comm.print("read all");
...
fwrite(x[,c("a","un")], file=paste("16384_0-1/outL",myrank,sep="."), sep=";",col.names=FALSE,quote=FALSE,append=F);
barrier()
