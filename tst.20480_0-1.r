...
#Rprof(append = TRUE)
myrank=comm.rank();
fnamev=paste("20480_0-1/outV",myrank,sep=".");

x = comm.fread ("auth20480", pattern="*",readers=1280, quote="",sep=";",header=F)
barrier()
comm.print("read all");
...
fwrite(x[,c("a","un")], file=paste("20480_0-1/outL",myrank,sep="."), sep=";",col.names=FALSE,quote=FALSE,append=F);
barrier()
