...
myrank=comm.rank();
fnamev=paste("4096_621-920/outV",myrank,sep=".");
ncom = comm.size();
nc = ceiling(ncom/2);
...
fwrite(x[,c("a","un")], file=paste("4096_621-920/outL",myrank,sep="."), sep=";",col.names=FALSE,quote=FALSE,append=F);
barrier()
dif = proc.time() - ptm;
comm.print(dif);
...
if (FR == 0){
  ...
  comm.print(str);
  #fwrite(pairs$data1[,c("a","un")],file=paste("4096_621-920/outL1",myrank,sep="."), sep=";",col.names=FALSE,quote=FALSE,append=F); 
  #barrier()
